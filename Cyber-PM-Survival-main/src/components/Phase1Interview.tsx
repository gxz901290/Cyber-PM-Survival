...
export const Phase1Interview: React.FC<Phase1Props> = ({ state, updateState, onComplete }) => {
  ...
  const dims = ['logic', 'communication', 'logic'];
  const attrPatterns = [
    ['logic', 'cronyism', 'sanity'],
    ['sanity', 'communication', 'owner'],
    ['cronyism', 'logic', 'cronyism']
  ];
  const scorePatterns = [
    [1.5, 0.5, 0],
    [0.5, 1.5, 1.0],
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      try {
          ...
          dims.map(async (dim, index) => {
            try {
              const res = await fetch('/.netlify/functions/generateQuestion', {
                ...
                body: JSON.stringify({ stage: 'Phase1Interview', dim })
              });
              ...
              return {
                ...
                options: data.options.map((text: string, i: number) => ({
                  text,
                  score: scorePatterns[index][i],
                  attr: attrPatterns[index][i]
                }))
              };
            } catch {
              return null;
        ...
        if (validQuestions.length === dims.length) {
  ...
  const handleAnswer = (option: typeof FALLBACK_QUESTIONS[0]['options'][0]) => {
    playClick();
      ...
      <div className="flex-1 flex flex-col justify-center max-w-2xl mx-auto w-full">
        {loading ? (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="cyber-glass p-6 rounded-xl border border-[#00E5FF]/30 mb-8 text-center"
          >
            <div className="flex items-center justify-center space-x-4 mb-6">
              <Loader className="animate-spin text-[#00E5FF]" />
              <div>
                <h3 className="text-[#00E5FF] font-bold uppercase text-sm tracking-widest">赛博面试官</h3>
                <p className="text-gray-400 text-xs font-mono">ID: HR-9000</p>
              </div>
            </div>
            <p className="text-xl font-medium leading-relaxed">赛博面试官正在出题...</p>
          </motion.div>
        ) : (
          <>
            <motion.div
